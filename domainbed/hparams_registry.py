...
def _hparams(algorithm, dataset, random_seed):
    ...
    _hparam("vit_dropout", 0.0, lambda r: r.choice([0.0, 0.1, 0.5]))
    ...
    if algorithm in ["DANN", "CDANN"]:
        ...
        if dataset == "DomainNet":
            _hparam(
                "rdm_penalty_anneal_iters", 2400, lambda r: int(r.uniform(1500, 3000))
            )
        else:
            _hparam(
                "rdm_penalty_anneal_iters", 1500, lambda r: int(r.uniform(800, 2700))
            )
        ...
        algorithm == "MMD"
        or algorithm == "CORAL"
        or algorithm == "CausIRL_CORAL"
        or algorithm == "CausIRL_MMD"
    ):
        ...
        _hparam("k", 1e1, lambda r: 10 ** r.uniform(-3, 5))
        ...
        _hparam(
            "lmbda", 1e-1, lambda r: r.choice([1e-4, 1e-3, 1e-2, 1e-1, 1, 1e1, 1e2])
        )
        ...
        _hparam(
            "urm", "adversarial", lambda r: str(r.choice(["adversarial"]))
        )  # 'adversarial'

        _hparam("urm_adv_lambda", 0.1, lambda r: float(r.uniform(0, 0.2)))
        _hparam(
            "urm_discriminator_label_smoothing", 0, lambda r: float(r.uniform(0, 0))
        )
        _hparam(
            "urm_discriminator_optimizer", "adam", lambda r: str(r.choice(["adam"]))
        )
        _hparam(
            "urm_discriminator_hidden_layers", 1, lambda r: int(r.choice([1, 2, 3]))
        )
        _hparam(
            "urm_generator_output", "tanh", lambda r: str(r.choice(["tanh", "relu"]))
        )
    ...
    if algorithm == "ADRMX":
        ...
        _hparam("mlp_dropout", 0.0, lambda r: r.choice([0]))

    elif algorithm == "MyModel":
        _hparam(
            "backbone",
            "ResNet",
            lambda r: r.choice(["ResNet", "ViT", "EfficientNet", "AlexNet"]),
        )
        _hparam("mlp_width", 256, lambda r: r.choice([256, 512]))
        _hparam("mlp_depth", 2, lambda r: int(r.choice([2, 3])))
        _hparam("mlp_dropout", 0.0, lambda r: r.choice([0.0, 0.1]))
